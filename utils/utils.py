...
def save_checkpoint_ema(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler,
                        ...
                        save_all=False, name=None, min_ckpt_num=3,):
    ...
    if not save_all:
        old_epoch = epoch - min_ckpt_num
        old_ckpt = os.path.join(config.OUTPUT, f'ckpt_epoch_{old_epoch}.pth')
    ...
    if name is not None:
        torch.save(save_state, os.path.join(config.OUTPUT, 'latest.pth'))
