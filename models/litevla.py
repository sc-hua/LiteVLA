...
default_args = dict(
    ...
    depths=(2, 2, 11, 3), block_types=('GAB', 'GAB', 'LA', 'LA'), 
    stem_kernel=5, ds_exp=4, ds_kernel=3, ds_fuse="ff--", act='silu', dwc_kernel=5,
    attn_ratio=1/8, attn_kernel='elu1', attn_norm='mrms', mlp_ratio=3, head_norm='mrms',
    use_checkpoint=False, distillation=False,  # for training
    backbone=False, out_indices=None, pretrained=None,  # for backbone
...
"""
def LiteVLA_Tiny(**kwargs):
    return LiteVLA(dims=(32, 64, 128, 256), ds_fuse="ffff", **kwargs)

def LiteVLA_Small(**kwargs):
    return LiteVLA(dims=(40, 80, 160, 320), ds_fuse="fff-", **kwargs)

def LiteVLA_Normal(**kwargs):
    return LiteVLA(dims=(48, 96, 192, 384), ds_fuse="ff--", **kwargs)

def LiteVLA_Medium(**kwargs):
    return LiteVLA(dims=(56, 112, 224, 448), ds_fuse="f---", **kwargs)

def LiteVLA_Large(**kwargs):
    return LiteVLA(dims=(64, 128, 256, 512), ds_fuse="f---", **kwargs)


def create_litevla(name, **kwargs):
    name = name.lower()
    if name in ['tiny', 't']:
        return LiteVLA_Tiny(**kwargs)
    if name in ['small', 's']:
        return LiteVLA_Small(**kwargs)
    if name in ['normal', 'n']:
        return LiteVLA_Normal(**kwargs)
    if name in ['medium', 'm']:
        return LiteVLA_Medium(**kwargs)
    if name in ['large', 'l']:
        return LiteVLA_Large(**kwargs)
